...
export function getAttendanceRecords(meetingId?: string): AttendanceRecord[] {
  ...
  const targetMeetings = meetingId ? meetings.filter(m => m.id === meetingId) : meetings;

  targetMeetings.forEach(meeting => {
    // Simulate 70-90% attendance rate
    const attendanceRate = 0.7 + Math.random() * 0.2;
    const attendingUsers = users.slice(0, Math.floor(users.length * attendanceRate));
    ...
    attendingUsers.forEach((user, index) => {
      ...
      if (Math.random() < 0.8) {
        ...
        checkedInAt.setMinutes(checkedInAt.getMinutes() - (Math.random() * 10 + 5));
      } else {
        // Late: 1-30 minutes late
        checkedInAt.setMinutes(checkedInAt.getMinutes() + (Math.random() * 29 + 1));
      }
      ...
      records.push({
        ...
        location: {
          ...
          address: meeting.meeting_type === 'virtual' ? 'Remote Location' : 'PT Pupuk Kujang, Cikampek'
        },
        deviceInfo: `${Math.random() > 0.5 ? 'Android' : 'iOS'} ${Math.random() > 0.5 ? 'Mobile App' : 'Web Browser'}`
      });
    });
  });

  return records.sort((a, b) => new Date(b.checkedInAt).getTime() - new Date(a.checkedInAt).getTime());
}
...
export function getActiveQRCodes(): QRCodeData[] {
  ...
  return meetings
    ...
    .map(meeting => {
      const meetingDate = new Date(meeting.date_time);
      const expiresAt = new Date(meetingDate.getTime() + (meeting.duration + 30) * 60000); // 30 min buffer
      
      return {
        meetingId: meeting.id,
        meetingTitle: meeting.title,
        qrCodeUrl: `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=meeting-checkin-${meeting.id}-${Date.now()}`,
...
export function getAttendanceStats(meetingId?: string): AttendanceStats {
  ...
  const qrCodeScans = records.filter(r => r.checkInMethod === 'qr_code').length;
  const manualCheckIns = records.filter(r => r.checkInMethod === 'manual').length;
  const autoCheckIns = records.filter(r => r.checkInMethod === 'auto').length;
  
  const attendanceRate = totalInvited > 0 ? (totalAttendees / totalInvited) * 100 : 0;
  const punctualityRate = totalAttendees > 0 ? (onTimeAttendees / totalAttendees) * 100 : 0;
...
export function generateMeetingQRCode(meetingId: string): QRCodeData | null {
  ...
  const meetingDate = new Date(meeting.date_time);
  const expiresAt = new Date(meetingDate.getTime() + (meeting.duration + 30) * 60000);
  const now = new Date();
  
  return {
    meetingId: meeting.id,
    meetingTitle: meeting.title,
    qrCodeUrl: `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=meeting-checkin-${meeting.id}-${Date.now()}`,
    expiresAt: expiresAt.toISOString(),
    isActive: now < expiresAt,
    scansCount: 0,
...
export function processQRCodeCheckIn(
  ...
  if (alreadyCheckedIn) {
    return { success: false, message: 'Anda sudah melakukan check-in untuk meeting ini' };
  }
  ...
  return {
    success: true,
    message: `Check-in berhasil! ${isLate ? 'Anda terlambat.' : 'Anda tepat waktu.'}`,
    record
  };
}
...
export function processManualCheckIn(
  ...
  if (admin.role !== 'admin' && admin.role !== 'manager') {
    return { success: false, message: 'Tidak memiliki permission untuk manual check-in' };
  }
...
export function getAttendanceByDepartment(): { department: string; stats: AttendanceStats }[] {
  const users = mockUsers;
  ...
  return departments.map(department => {
    ...
    const deptRecords = getAttendanceRecords().filter(r => deptUserIds.includes(r.userId));
    ...
    return {
      ...
      stats: {
        ...
        qrCodeScans: deptRecords.filter(r => r.checkInMethod === 'qr_code').length,
        manualCheckIns: deptRecords.filter(r => r.checkInMethod === 'manual').length,
        autoCheckIns: deptRecords.filter(r => r.checkInMethod === 'auto').length,
        attendanceRate: 85, // Mock data
        punctualityRate: totalAttendees > 0 ? (onTimeAttendees / totalAttendees) * 100 : 0
      }
    };
  });
}
